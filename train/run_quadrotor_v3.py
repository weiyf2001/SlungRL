...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Train PPO v3 with an LSTM feature extractor.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Load a pretrained v3 model ID from runs/v3/<id>/checkpoints.",
    )
...
def make_env(render_mode, seed):
    def _init():
        return QuadrotorPayloadEnvV2(render_mode=render_mode, env_num=seed)
...
def main():
    ...
    run_paths = get_run_directories("v3", experiment_id)
    ...
    env_fns = [make_env(render_mode, seed=i) for i in range(num_envs)]
    ...
    if args_dict["checkpoint"] is not None:
        checkpoint_path = get_checkpoint_path("v3", args_dict["checkpoint"])
