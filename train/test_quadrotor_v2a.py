...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Evaluate PPO v2a with hidden payload parameters.")
...
def main():
    ...
    model_path = get_checkpoint_path("v2a", args_dict["id"])
    loaded_model = PPO.load(model_path)

    render_mode = "human" if args_dict["visualize"] else None
    env = VecMonitor(DummyVecEnv([lambda: QuadrotorPayloadEnvV2A(render_mode=render_mode)]))
